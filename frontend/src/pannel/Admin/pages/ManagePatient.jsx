  ...
  const filteredPatients = useMemo(() => {
    ...
    return patients.filter((patient) => {
      ...
      if (statusFilter === "active") statusMatch = patient.isActive === true;
        ...
        statusMatch = patient.isActive === false;
    ...
    <div className="space-y-6 max-w-400 mx-auto animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-slate-400 text-sm">
            Review and manage all registered patient accounts
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm"
          >
            <div className="flex items-center gap-4">
              <div
                className={`p-3 rounded-xl bg-${stat.color}-50 dark:bg-${stat.color}-500/10`}
              >
                <stat.icon
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800">
                  <th className="px-6 py-4 text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">
                    Patient
                  </th>
                  <th className="px-6 py-4 text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest hidden sm:table-cell">
                    Contact
                  </th>
                  <th className="px-6 py-4 text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest hidden md:table-cell">
                    Gender
                  </th>
                  <th className="px-6 py-4 text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest hidden lg:table-cell">
                    Joined
                  </th>
                  <th className="px-6 py-4 text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500 dark:text-slate-400 uppercase font-bold tracking-tight sm:hidden">
                            {patient.email}
                          </p>
                        </div>
                      </div>
      ...
      {showDetailModal && selectedPatient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-4xl shadow-2xl overflow-hidden relative border border-gray-100 dark:border-slate-800 animate-in slide-in-from-bottom-5 duration-300">
            {/* Profile Background */}
            <div
              className={`h-32 bg-linear-to-r from-indigo-500 to-sky-500 dark:opacity-80`}
            />
            ...
            <div className="px-8 pb-8 relative">
              <div className="flex justify-between items-end -mt-12 mb-6">
                ...
                <div className="flex gap-2 mb-2">
                  {selectedPatient.isActive !== false ? (
                    <span className="px-3 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                      Active Account
                    </span>
                  ) : (
                    <span className="px-3 py-1 bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                      Inactive
                    </span>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-6 my-8 pb-8 border-b border-gray-100 dark:border-slate-800">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p
                        className="text-sm font-bold text-gray-700 dark:text-slate-200 truncate max-w-35"
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-700 dark:text-slate-200 capitalize">
                        {selectedPatient.gender?.toLowerCase() || "Unspecified"}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-8 flex gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-2xl text-[13px] font-bold transition-all border border-red-100/50 dark:border-red-500/20"
                >
                  Delete Account
                </button>
                <button
                  onClick={() => setShowDetailModal(false)}
                  className="flex-1 py-3.5 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-700 dark:text-white rounded-2xl text-[13px] font-bold transition-all"
                >
                  Close
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-800 p-8 text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 font-bold text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-2xl transition-all"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-bold transition-all shadow-lg shadow-red-500/20 active:scale-95"
              >
                Delete Permanently
              </button>
            </div>
